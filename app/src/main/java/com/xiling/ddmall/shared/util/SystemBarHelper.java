...
public class SystemBarHelper {
    private static float DEFAULT_ALPHA = Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP ? 0f : 0.3f;
    ...
    public static void tintStatusBar(Window window, @ColorInt int statusBarColor, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        ...
        if (isEMUI3_1() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            // EMUI 3.1 window.setStatusBarColor(Color.TRANSPARENT) is not work
    ...
    public static void setStatusBarDarkMode(Window window) {
        if (isFlyme4Later()) {
            ...
            tintStatusBar(window, window.getContext().getResources().getColor(R.color.bg_gray), DEFAULT_ALPHA);
        }
    }
    ...
    public static void statusBarLightMode(Activity activity) {
        if (isFlyme4Later()) {
            setStatusBarDarkModeForFlyme4(activity.getWindow(), false);
        } else if (isMIUI6Later()) {
            setStatusBarDarkModeForMIUI6(activity.getWindow(), false);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            setStatusBarDarkModeForM(activity.getWindow(), false);
        } else {
            tintStatusBar(activity.getWindow(), activity.getResources().getColor(R.color.bg_gray), DEFAULT_ALPHA);
        }
    }
    ...
    public static void setStatusBarDarkModeForM(Window window, boolean isDark) {
        window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
        window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
        window.setStatusBarColor(Color.TRANSPARENT);
        int systemUiVisibility = window.getDecorView().getSystemUiVisibility();
